...
```{r,echo=FALSE,warning=FALSE,fig.show=TRUE,fig.height=5,fig.width=5}
options(warn=-1)
suppressPackageStartupMessages(library(ggplot2));suppressPackageStartupMessages(library(UsingR))
data(Galton)
p=qplot(parent, child, data = Galton, geom = c("point"))
p=p+geom_smooth(method="lm",formula= y~x,lwd=1.5)
print(p)
